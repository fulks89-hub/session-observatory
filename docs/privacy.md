...
Explicitly sending a Codex follow-up launches the installed Codex CLI for the selected idle conversation, using its configured provider, authentication, tools, and existing rules. Its resumed history and prompt may leave the machine according to that native configuration. The app does not extract native credentials. The read-only sandbox constrains sandboxed execution; it is not a blanket guarantee about independently configured MCP tools. Claude and Cursor drafts are not sent by the app. Follow-up prompts and bounded response receipts are stored locally; copying a draft places it on the system clipboard.

Skill-lab model credentials are read from the launching environment and sent only in the configured provider's request headers. Endpoint user/password credentials are rejected, redirects are rejected, and remote endpoints require HTTPS. Use headers rather than putting secrets in URLs. Provider billing, retention, and data handling are governed by that provider and the operator's account configuration.
